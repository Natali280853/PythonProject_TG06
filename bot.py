...
# При получении сообщения мы можем получить информацию и сохранить ее в переменных: ID пользователя и имя.
@dp.message(F.text == "Регистрация в телеграм боте")
async def registration(message: Message):
    telegram_id = message.from_user.id
    name = message.from_user.full_name
    # знак ? в selecte - это специфика sqlite. так передается в where значение
    cursor.execute('''SELECT * FROM users WHERE telegram_id = ?''', (telegram_id,))
    # Переменная user = cursor.fetchone() будет брать первый попавшийся результат, но он и будет один.
...
async def exchange_rates(message: Message):
    ...
    try:
        ...
        # “.2f” - формат строки, чтобы число было с двумя знаками после запятой
